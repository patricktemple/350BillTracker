...
from src.person.models import CouncilMember, OfficeContact, Person, Senator, AssemblyMember
...
@pytest.mark.parametrize(
    ("senate_bill_exists", "assembly_bill_exists"), [(False, True), (True, True), (True, False)]
)
@patch("src.google_sheets.Credentials")
@patch("src.google_sheets.build")
def test_create_state_power_hour__no_import(
    mock_build, mock_credentials, snapshot, state_bill, senator, assembly_member, senate_bill_exists, assembly_bill_exists
):
    mock_sheets_service = MagicMock()
    mock_drive_service = MagicMock()
    ...
    if senate_bill_exists:
        non_sponsor_senator = Person(
            id=uuid4(), name="Non sponsor senator", type=Person.PersonType.SENATOR
        )
    ...
    if assembly_bill_exists:
        non_sponsor_assembly_member = Person(
            id=uuid4(), name="Non sponsor assembly member", type=Person.PersonType.ASSEMBLY_MEMBER
        )
        non_sponsor_assembly_member.assembly_member = AssemblyMember(state_member_id=2)
