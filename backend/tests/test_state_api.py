...
from src.bill.models import StateBill, Bill
from src.person.models import Person, Senator, AssemblyMember
from src.sponsorship.models import SenateSponsorship, AssemblySponsorship
...
def test_update_state_bills(state_bill: Bill, senator, assembly_member):
    ...
    db.session.add(SenateSponsorship(person_id=senator.id, bill_id=state_bill.id, is_lead_sponsor=False))
    db.session.add(AssemblySponsorship(person_id=assembly_member.id, bill_id=state_bill.id, is_lead_sponsor=False))
    db.session.commit()

    different_senate_person = Person(name="Different senator", type=Person.PersonType.SENATOR)
    different_senate_person.senator = Senator(state_member_id=100)
    db.session.add(different_senate_person)

    different_assembly_person = Person(name="Different assembly member", type=Person.PersonType.ASSEMBLY_MEMBER)
    different_assembly_person.assembly_member = AssemblyMember(state_member_id=200)
    ...
    assert state_bill.state_bill.senate_bill.active_version == "New senate version"
    ...
    assert state_bill.state_bill.assembly_bill.active_version == "New assembly version"
    ...
    assert assembly_sponsorship.representative.person.name == "Different assembly member"
